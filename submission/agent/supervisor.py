...
def guard_main(obs_dict, select: Select, gd: GameData, choice: list[int]) -> list[int]:
    """
    Prevent the autonomous stack from ignoring an immediately winning attack.
    If a game-winning attack is already available, there is no strategic value
    in taking any other action first. Override any non-winning choice with the
    best winning attack. Otherwise leave the original decision untouched.
    """
    try:
        ...
        if not lethals:
            return choice
        # Best lethal = most prizes, then most damage.
        lethals.sort(key=lambda t: (t[2], t[1]), reverse=True)
        best_attack = lethals[0][0]
        # Already chose a winning attack.
        if choice[0] == best_attack:
            return choice
        return [best_attack]
    except Exception:
        return choice