"""Engine enum constants (mirrors cabt ``api`` integer enums).

The engine sends enum values as plain integers in the observation dict, so we
keep a dependency-free copy here rather than importing ``cg.api`` (which is only
present at battle time). Values are taken verbatim from the cabt API reference.
"""
